...
async def background_generate_product_image(toolbox: Toolbox, prompt: str, count: int, product_id: str, generation_job_id: str):
    try:
        # Get the generation job and product from the database
        generation_job = await GenerationJob.get(generation_job_id)
        product = await Product.get(product_id)

        if not generation_job or not product:
            raise ValueError("Generation job or product not found")
        ...
        image_data = await image_service.generate_images(prompt, count, product_id, generation_job_id)
        ...
        for index, image_datum in enumerate(image_data):
            ...
            generated_image = await GeneratedImage.create(generation_job_id=generation_job.id, url=image_url)
